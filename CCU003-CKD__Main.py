...
# MAGIC ### Functions
# MAGIC 
# MAGIC **Author** Muhammad Dashtban (Ashkan) - 
...
def save(df,tableName="ckd_med_table"):
  ...
  spark.sql(f"""CREATE TABLE .{tableName} as SELECT * FROM global_temp.{tableName}""")
  spark.sql(f"""ALTER TABLE .{tableName} OWNER TO """)
  
#drop intermediate table or other tables
def rm(tableName="ckd_med_table",dbname=""):
...
def loadhes(exit_date="2020-12-01"):
  hes=spark.table(f""".hes_apc_all_years""")
...
def loadcovid(covid="covid",cov_date="c_date",cov_status="c_status",cov_month="c_month",covid_flag="covid"):
  ...
  cov = spark.table('.ccu002_covid19_test_all')
